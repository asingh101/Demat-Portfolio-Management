    ...
    <div class="content_bg">
      <div class="mainbar">
        <div class="article">
         ...
         ArrayList<LoginAction> alist=(ArrayList)sess.get("salerequest"); 
         for (int i=0;i<alist.size();i++)
         {
          %>
         <tr><td><%=alist.get(i).getCompname()%></td><td><%=alist.get(i).getNo_share()%></td><td><%=alist.get(i).getUsername()%></td>
         <td><a href="saleprocess?compname=<%=alist.get(i).getCompname()%>&no_share=<%=alist.get(i).getNo_share()%>&username=<%=alist.get(i).getUsername()%>&id=<%=alist.get(i).getId()%>&cid=<%=alist.get(i).getCid()%>&userid=<%=alist.get(i).getUserid() %>&broker_id=<%=alist.get(i).getBroker_id()%>">Process</a></td></tr>
         <%} %>
         </table>
